...
void SceneReader::parseLightBlock(std::queue<std::string>& tokenBlock) {
	...
	if (lightType == "POINTLIGHT") {
		...
		Direction direction (parseNumber(tokenBlock), parseNumber(tokenBlock), parseNumber(tokenBlock));
		double angle = parseNumber(tokenBlock);
		light = scene_->newLight<SpotLightSource>(direction, angle);
	} else {
...
void SceneReader::parseObjectBlock(std::queue<std::string>& tokenBlock) {
	...
	if (objectType == "SPHERE") {
	  ...
	  object = scene_->newObject<Cylinder>();
	} else {
